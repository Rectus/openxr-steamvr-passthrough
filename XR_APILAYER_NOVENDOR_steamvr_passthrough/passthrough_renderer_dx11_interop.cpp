...
static PFN_glGenSemaphoresEXT glGenSemaphoresEXT = nullptr;
static PFN_glImportSemaphoreWin32HandleEXT glImportSemaphoreWin32HandleEXT = nullptr;
static PFN_glSemaphoreParameterui64vEXT glSemaphoreParameterui64vEXT = nullptr;
static PFN_glWaitSemaphoreEXT glWaitSemaphoreEXT = nullptr;
static PFN_glSignalSemaphoreEXT glSignalSemaphoreEXT = nullptr;

static PFN_glCreateMemoryObjectsEXT glCreateMemoryObjectsEXT = nullptr;
static PFN_glDeleteMemoryObjectsEXT glDeleteMemoryObjectsEXT = nullptr;
static PFN_glImportMemoryWin32HandleEXT glImportMemoryWin32HandleEXT = nullptr;
static PFN_glTexStorageMem2DEXT glTexStorageMem2DEXT = nullptr;

static PFN_glCopyImageSubData glCopyImageSubData = nullptr;
...
{
	glGenSemaphoresEXT = reinterpret_cast<PFN_glGenSemaphoresEXT>(wglGetProcAddress("glGenSemaphoresEXT"));
	glImportSemaphoreWin32HandleEXT = reinterpret_cast<PFN_glImportSemaphoreWin32HandleEXT>(wglGetProcAddress("glImportSemaphoreWin32HandleEXT"));
	glSemaphoreParameterui64vEXT = reinterpret_cast<PFN_glSemaphoreParameterui64vEXT>(wglGetProcAddress("glSemaphoreParameterui64vEXT"));
	glWaitSemaphoreEXT = reinterpret_cast<PFN_glWaitSemaphoreEXT>(wglGetProcAddress("glWaitSemaphoreEXT"));
	glSignalSemaphoreEXT = reinterpret_cast<PFN_glSignalSemaphoreEXT>(wglGetProcAddress("glSignalSemaphoreEXT"));

	glCreateMemoryObjectsEXT = reinterpret_cast<PFN_glCreateMemoryObjectsEXT>(wglGetProcAddress("glCreateMemoryObjectsEXT"));
	glDeleteMemoryObjectsEXT = reinterpret_cast<PFN_glDeleteMemoryObjectsEXT>(wglGetProcAddress("glDeleteMemoryObjectsEXT"));
	glImportMemoryWin32HandleEXT = reinterpret_cast<PFN_glImportMemoryWin32HandleEXT>(wglGetProcAddress("glImportMemoryWin32HandleEXT"));
	glTexStorageMem2DEXT = reinterpret_cast<PFN_glTexStorageMem2DEXT>(wglGetProcAddress("glTexStorageMem2DEXT"));

	glCopyImageSubData = reinterpret_cast<PFN_glCopyImageSubData>(wglGetProcAddress("glCopyImageSubData"));
}
...
bool PassthroughRendererDX11Interop::InitRenderer()
{
	...
	case RenderAPI_OpenGL:
	{
		...
		for (uint32_t i = 0; i < deviceCount; i++)
		{
			...
			LUID luid = *reinterpret_cast<LUID*>(deviceIDProps.deviceLUID);

			if (deviceIDProps.deviceLUIDValid && luid.LowPart == adapterDesc.AdapterLuid.LowPart && luid.HighPart == adapterDesc.AdapterLuid.HighPart)
			{
				m_vulkanPhysDevice = devices[i];
...
bool PassthroughRendererDX11Interop::InitVulkanDownloadTexture()
{
...
bool PassthroughRendererDX11Interop::CreateLocalTextureOpenGL(uint32_t& localOpenGLTexture, uint32_t& localOpenGLMemory, ID3D11Texture2D** localD3DTexture, HANDLE& sharedTextureHandle, const XrSwapchainCreateInfo& swapchainInfo, bool bIsDepthMap)
{
	DXGI_FORMAT texFormat = OpenGLImageFormatToDXGI(swapchainInfo.format);

	D3D11_TEXTURE2D_DESC textureDesc = {};
	textureDesc.Format = bIsDepthMap ? DXGI_DepthFormatToTypeless(texFormat) : texFormat;
	textureDesc.Width = swapchainInfo.width;
	textureDesc.Height = swapchainInfo.height;
	textureDesc.ArraySize = swapchainInfo.arraySize;
	textureDesc.MipLevels = swapchainInfo.mipCount;
	...
	if (localOpenGLMemory != 0)
	{
		glDeleteMemoryObjectsEXT(1, &localOpenGLMemory);
		localOpenGLMemory = 0;
	}
	...
	if (localOpenGLTexture != 0)
	{
		...
		localOpenGLTexture = 0;
	}

	glGenTextures(1, &localOpenGLTexture);
	glBindTexture(GL_TEXTURE_2D, localOpenGLTexture);
	...
	glCreateMemoryObjectsEXT(1, &localOpenGLMemory);
	...
	glImportMemoryWin32HandleEXT(localOpenGLMemory, 0, HANDLE_TYPE_D3D11_IMAGE_KMT_EXT, static_cast<void*>(sharedTextureHandle));
	...
	glTexStorageMem2DEXT(GL_TEXTURE_2D, 1, static_cast<GLenum>(swapchainInfo.format), swapchainInfo.width, swapchainInfo.height, localOpenGLMemory, 0);
...
void PassthroughRendererDX11Interop::RenderPassthroughFrame(const XrCompositionLayerProjection* layer, std::shared_ptr<CameraGPUFrame> frame, FrameRenderParameters& renderParams, std::shared_ptr<DepthFrame> depthFrame, UVDistortionParameters& distortionParams)
{
	...
	{
		{
			...
			if (layer->views[0].subImage.swapchain != XR_NULL_HANDLE && m_rendertargetsOpenGLLeft[renderParams.LeftFrameIndex] > 0)
			{
				...
				uint32_t sourceTexture =  m_rendertargetsOpenGLLeft[renderParams.LeftFrameIndex];
				uint32_t destTexture = m_localRendertargetsOpenGLLeft[renderParams.LeftFrameIndex];
			...
			if (layer->views[1].subImage.swapchain != XR_NULL_HANDLE && m_rendertargetsOpenGLRight[renderParams.RightFrameIndex] > 0)
			{
				...
				uint32_t sourceTexture =  m_rendertargetsOpenGLRight[renderParams.RightFrameIndex];
				uint32_t destTexture = m_localRendertargetsOpenGLRight[renderParams.RightFrameIndex];
			...
			{
				const XrRect2Di& imageRect = layer->views[0].subImage.imageRect;
				uint32_t sourceTexture = m_depthBuffersOpenGLLeft[renderParams.LeftDepthIndex];
				uint32_t destTexture = m_localDepthBuffersOpenGLLeft[renderParams.LeftDepthIndex];
			...
			{
				const XrRect2Di& imageRect = layer->views[1].subImage.imageRect;
				uint32_t sourceTexture =  m_depthBuffersOpenGLRight[renderParams.RightDepthIndex];
				uint32_t destTexture = m_localDepthBuffersOpenGLRight[renderParams.RightDepthIndex];
		...
		{
			const XrRect2Di& imageRect = layer->views[0].subImage.imageRect;
			uint32_t sourceTexture = m_localRendertargetsOpenGLLeft[renderParams.LeftFrameIndex];
		...
		{
			const XrRect2Di& imageRect = layer->views[1].subImage.imageRect;
			uint32_t sourceTexture = m_localRendertargetsOpenGLRight[renderParams.RightFrameIndex];
		...
		{
			const XrRect2Di& imageRect = layer->views[0].subImage.imageRect;
			uint32_t sourceTexture = m_localDepthBuffersOpenGLLeft[renderParams.LeftDepthIndex];
		...
		{
			const XrRect2Di& imageRect = layer->views[1].subImage.imageRect;
			uint32_t sourceTexture = m_localDepthBuffersOpenGLRight[renderParams.RightDepthIndex];
...
bool PassthroughRendererDX11Interop::DownloadTextureToCPU(void* textureSRV, const uint32_t width, const uint32_t height, const uint32_t bufferSize, uint8_t* buffer)
{
	switch (m_applicationRenderAPI)
	{
	case RenderAPI_Vulkan:
	case RenderAPI_OpenGL: // Use Vulkan context for OpenGL apps, since OpenGL doesn't support multithreading with separate rendering contexts on Windows.
	{
		...
		ID3D11Resource* res;
		static_cast<ID3D11ShaderResourceView*>(textureSRV)->GetResource(&res);
