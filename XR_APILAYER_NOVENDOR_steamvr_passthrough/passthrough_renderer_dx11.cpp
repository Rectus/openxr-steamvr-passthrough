...
#include "shaders\alpha_test_ps.h"
...
void UploadTexture(ComPtr<ID3D11DeviceContext> deviceContext, ComPtr<ID3D11Texture2D> uploadTexture, uint8_t* inputBuffer, VkExtent2D textureSize, uint32_t bitsPerPixel)
{
	...
	if (deviceContext->Map(uploadTexture.Get(), 0, D3D11_MAP_WRITE, 0, &resource) == S_OK)
	{
		...
		{
			memcpy(writePtr, readPtr, textureSize.width * bitsPerPixel);
			writePtr += resource.RowPitch;
			readPtr += textureSize.width * bitsPerPixel;
		}
...
bool PassthroughRendererDX11::InitRenderer()
{
	...
	{
		g_logger->error("g_FullscreenQuadVS creation failure!");
		return false;
	}
	...
	{
		g_logger->error("g_MeshRigidVS creation failure!");
		return false;
	}
	...
	{
		g_logger->error("g_PassthroughVS creation failure!");
		return false;
	}
	...
	{
		g_logger->error("g_PassthroughStereoVS creation failure!");
		return false;
	}
	...
	{
		g_logger->error("g_PassthroughPS creation failure!");
		return false;
	}
	...
	{
		g_logger->error("g_PassthroughTemporalPS creation failure.");
		return false;
	}
	SET_DXGI_DEBUGNAME(m_passthroughTemporalPS);

	if (FAILED(m_d3dDevice->CreatePixelShader(g_AlphaTestPS, sizeof(g_AlphaTestPS), nullptr, &m_alphaTestPS)))
	{
		g_logger->error("m_alphaTestPS creation failure!");
		return false;
	}
	SET_DXGI_DEBUGNAME(m_alphaTestPS);

	if (FAILED(m_d3dDevice->CreatePixelShader(g_AlphaPrepassPS, sizeof(g_AlphaPrepassPS), nullptr, &m_alphaPrepassPS)))
	{
		g_logger->error("g_AlphaPrepassPS creation failure!");
		return false;
	}
	...
	{
		g_logger->error("g_AlphaPrepassMaskedPS creation failure!");
		return false;
	}
	...
	{
		g_logger->error("g_AlphaPrepassMaskedFullscreenPS creation failure!");
		return false;
	}
	...
	{
		g_logger->error("g_AlphaCopyMaskedPS creation failure!");
		return false;
	}
	...
	{
		g_logger->error("g_depthWritePS creation failure!");
		return false;
	}
	...
	{
		g_logger->error("g_depthWriteTemporalPS creation failure!");
		return false;
	}
	...
	{
		g_logger->error("g_FullscreenPassthroughPS creation failure!");
		return false;
	}
	...
	{
		g_logger->error("g_FullscreenPassthroughTemporalPS creation failure!");
		return false;
	}
	...
	{
		g_logger->error("g_FullscreenPassthroughCompositePS creation failure!");
		return false;
	}
	...
	{
		g_logger->error("g_FullscreenPassthroughCompositeTemporalPS creation failure!");
		return false;
	}
	...
	{
		g_logger->error("g_debugAlphaToColorPS creation failure!");
		return false;
	}
	...
	{
		g_logger->error("g_debugDepthToColorPS creation failure!");
		return false;
	}
	...
	for (int i = 0; i < vr::k_unMaxTrackedDeviceCount; i++)
	{
		...
		{
			g_logger->error("m_vsMeshConstantBuffer creation failure!");
			return false;
		}
	}
	...
	{
		g_logger->error("CreateDepthStencilState failure!");
		return false;
	}
	...
	{
		g_logger->error("CreateSamplerState failure!");
		return false;
	}
	...
	{
		g_logger->error("CreateBlendState failure!");
		return false;
	}
	...
	{
		g_logger->error("CreateRasterizerState failure!");
		return false;
	}
	...
	{
		g_logger->error("CreateInputLayout failure!");
		return false;
	}
...
void PassthroughRendererDX11::SetupDebugTexture(DebugTexture& texture)
{
	...
	textureDesc.Width = texture.TextureSize.width;
	textureDesc.Height = texture.TextureSize.height;
	...
	{
		g_logger->error("Debug Texture CreateTexture2D error!");
		return;
	}
	...
	{
		g_logger->error("Debug Texture Upload CreateTexture2D error!");
		return;
	}
	...
	{
		g_logger->error("Debug Texture CreateShaderResourceView error!");
		return;
	}
}
...
{
	{
		...
		for (int i = 0; i < 2; i++)
		{
			...
			{
				g_logger->error("Passthrough Depth Stencil CreateTexture2D error!");
				return;
			}
			...
			{
				g_logger->error("Passthrough Depth Stencil CreateShaderResourceView error!");
				return;
			}
			...
			{
				g_logger->error("Passthrough Depth Stencil CreateDepthStencilView error!");
			}
		}
	}

	{
		...
		{
			g_logger->error("Passthrough Camera Invalidation CreateTexture2D error!");
			return;
		}
		...
		{
			g_logger->error("Passthrough Camera Invalidation CreateShaderResourceView error!");
			return;
		}
		...
		{
			g_logger->error("Passthrough Camera Invalidation CreateRenderTargetView error!");
		}
	}
}


void PassthroughRendererDX11::SetupUVDistortionMap(UVDistortionParameters& distortionParams)
{
	D3D11_TEXTURE2D_DESC textureDesc = {};
	textureDesc.MipLevels = 1;
	textureDesc.Format = DXGI_FORMAT_R32G32_FLOAT;
	textureDesc.Width = distortionParams.UVDistortionMapSize.width;
	textureDesc.Height = distortionParams.UVDistortionMapSize.height;
	textureDesc.ArraySize = 1;
	textureDesc.SampleDesc.Count = 1;
	textureDesc.SampleDesc.Quality = 0;
	textureDesc.BindFlags = D3D11_BIND_SHADER_RESOURCE;
	...
	uploadData.pSysMem = (void*)distortionParams.UVDistortionMap->data();
	uploadData.SysMemPitch = distortionParams.UVDistortionMapSize.width * 8; // 2 * 32 bits

	if (FAILED(m_d3dDevice->CreateTexture2D(&textureDesc, &uploadData, &m_uvDistortionMap.Texture)))
	{
		g_logger->error("UV Distortion Map CreateTexture2D error!");
		return;
	}
	...
	{
		g_logger->error("UV Distortion Map CreateShaderResourceView error!");
		return;
	}
}
...
DX11TemporaryRenderTarget& PassthroughRendererDX11::GetTemporaryRenderTarget(const uint32_t swapchainIndex, const uint32_t eyeIndex)
{
	...
	{
		g_logger->error("Temporary Render Target CreateTexture2D error!");
		return viewData.temporaryRenderTarget;
	}
	...
	{
		g_logger->error("Temporary Render Target CreateShaderResourceView error!");
	}
	...
	{
		g_logger->error("Temporary Render Target CreateRenderTargetView error!");
	}
...
void PassthroughRendererDX11::InitRenderTarget(const ERenderEye eye, void* rendertarget, const uint32_t imageIndex, const XrSwapchainCreateInfo& swapchainInfo, const XrSwapchain swapchain)
{
	...
	{
		g_logger->error("Failed to init DX11 frame data!");
		return;
	}

	if (!CheckInitViewData(viewIndex, imageIndex))
	{
		g_logger->error("Failed to init DX11 view data!");
		return;
	}
	...
	{
		g_logger->error("Render Target CreateRenderTargetView error!");
		return;
	}
	...
	{
		g_logger->error("Render Target CreateShaderResourceView error!");
		return;
	}
...
bool PassthroughRendererDX11::CheckInitFrameData(const uint32_t imageIndex)
{
	...
	{
		g_logger->error("m_psMaskedConstantBuffer creation failure!");
		return false;
	}
...
void PassthroughRendererDX11::SetupTemporalUAV(const uint32_t viewIndex, const uint32_t frameIndex, const uint32_t width, const uint32_t height)
{
	...
	{
		g_logger->error("UAV CreateTexture2D error!");
		return;
	}
	...
	{
		g_logger->error("UAV CreateUnorderedAccessView error!");
		return;
	}
	...
	{
		g_logger->error("UAV CreateShaderResourceView error!");
		return;
	}
}
...
void PassthroughRendererDX11::InitDepthBuffer(const ERenderEye eye, void* depthBuffer, const uint32_t imageIndex, const XrSwapchainCreateInfo& swapchainInfo, const XrSwapchain swapchain)
{
	...
	{
		g_logger->error("Depth map CreateDepthStencilView error!");
		return;
	}
...
void PassthroughRendererDX11::GenerateMesh()
{
	...
	{
		g_logger->error("Mesh vertex buffer creation error!");
		return;
	}
	...
	{
		g_logger->error("Mesh index buffer creation error!");
		return;
	}
...
void PassthroughRendererDX11::GenerateDepthMesh(uint32_t width, uint32_t height)
{
	...
	{
		g_logger->error("Depth mesh vertex buffer creation error!");
		return;
	}
	...
	{
		g_logger->error("Depth mesh index buffer creation error!");
		return;
	}
		...
		for (DX11RenderModel& dxModel : m_renderModels)
		{
			if (model.deviceId == dxModel.deviceId)
			{
				...
				if (!dxModel.meshPtr || &model.mesh != dxModel.meshPtr)
				{
					...
					{
						g_logger->error("Render model vertex buffer creation error!");
					}
					...
					{
						g_logger->error("Render model index buffer creation error!");
					}
...
void PassthroughRendererDX11::RenderPassthroughFrame(const XrCompositionLayerProjection* layer, std::shared_ptr<CameraGPUFrame> frame, FrameRenderParameters& renderParams, std::shared_ptr<DepthFrame> depthFrame, UVDistortionParameters& distortionParams)
{
	...
	if (renderParams.bIsFirstRenderOfCameraFrame) { m_currentCameraFilterIndex = m_currentCameraFilterIndex == 0 ? 1 : 0; }
	...
	if (renderParams.ProjectionMode == Projection_StereoReconstruction)
	{
		...
		if (depthFrame->OutputDisparityTextureSize.width != m_disparityMapWidth || (stereoConf.StereoUseDisparityTemporalFiltering && frameData.disparityFilter.Texture == nullptr))
		{
			m_disparityMapWidth = depthFrame->OutputDisparityTextureSize.width;

			if (stereoConf.StereoUseDisparityTemporalFiltering)
			{
				SetupDisparityFilter(depthFrame->OutputDisparityTextureSize.width, depthFrame->OutputDisparityTextureSize.height);
			}
		...
		if(viewDataLeft.passthroughDepthStencil[0].Texture == nullptr || 
			...
			(viewDataLeft.passthroughDepthStencil[0].Width != depthFrame->OutputDisparityTextureSize.width * stereoConf.StereoDepthMapScale / 2) ||
			(viewDataLeft.passthroughDepthStencil[0].Height != depthFrame->OutputDisparityTextureSize.height * stereoConf.StereoDepthMapScale))
		{
			uint32_t width = depthFrame->OutputDisparityTextureSize.width * stereoConf.StereoDepthMapScale / 2;
			uint32_t height = depthFrame->OutputDisparityTextureSize.height * stereoConf.StereoDepthMapScale;
		...
		if (m_bUseHexagonGridMesh != stereoConf.StereoUseHexagonGridMesh || bDisparityMapReset || bDepthMapsReset)
		{
			...
			GenerateDepthMesh(depthFrame->OutputDisparityTextureSize.width * stereoConf.StereoDepthMapScale / 2, depthFrame->OutputDisparityTextureSize.width * stereoConf.StereoDepthMapScale);
		}
	}

	VSPassConstantBuffer vsBuffer{};

	vsBuffer.worldToCameraFrameProjectionLeft = renderParams.CameraLeft.WorldToProjection;
	vsBuffer.worldToCameraFrameProjectionRight = renderParams.CameraRight.WorldToProjection;
	...
	if (renderParams.ProjectionMode == Projection_StereoReconstruction)
	{
		...
		vsBuffer.disparityTextureSize[0] = depthFrame->OutputDisparityTextureSize.width;
		vsBuffer.disparityTextureSize[1] = depthFrame->OutputDisparityTextureSize.height;
	...
	if (mainConf.DebugSource == DebugSource_ApplicationAlpha || 
		mainConf.DebugSource == DebugSource_ApplicationDepth)
	{
	...
	if (mainConf.DebugTexture != DebugTexture_None)
	{
		...
		if (texture.CurrentTexture == mainConf.DebugTexture)
		{
			...
			{
				UploadTexture(m_deviceContext, m_debugTextureUpload, texture.Texture.data(), texture.TextureSize, texture.PixelSize);
	...
	m_renderContext->RSSetState(renderParams.bMirrorRender ? m_rasterizerStateMirrored.Get() : m_rasterizerState.Get());
	...
	psPassBuffer.worldToCameraFrameProjectionLeft = renderParams.CameraLeft.WorldToProjection;
	psPassBuffer.worldToCameraFrameProjectionRight = renderParams.CameraRight.WorldToProjection;
	...
	psPassBuffer.opacity = fminf(coreConf.CoreForcePassthroughOpacity, renderParams.RenderOpacity);
	psPassBuffer.alphaTestThreshold = coreConf.CoreForceAlphaTestTreshold;
	...
	psPassBuffer.bIsFirstRenderOfCameraFrame = renderParams.bIsFirstRenderOfCameraFrame;
	...
	psPassBuffer.bHasReversedDepth = renderParams.bHasReversedDepth;
	...
	bool bRenderBackground = stereoConf.StereoDrawBackground && renderParams.ProjectionMode == Projection_StereoReconstruction && !mainConf.DebugStereoReconstructionFreeze && !renderParams.bEnableDepthRange && !m_configManager->GetConfig_Camera().ClampCameraFrame;

	bool bRenderAlphaPrepass =
		(renderParams.bInvertLayerAlpha && (renderParams.ProjectionMode != Projection_StereoReconstruction || renderParams.BlendMode == Masked)) || // Use prepass for inverting alpha channel
		...
		(renderParams.BlendMode != Masked && renderParams.bEnableDepthBlending && renderParams.ProjectionMode != Projection_StereoReconstruction) || // Use prepass for cutting out depth
		(coreConf.CoreForcePassthroughOpacity < 1.0f && renderParams.BlendMode != AlphaTest); // Use prepass to lower opacity
	...
	if (renderParams.LeftFrameIndex >= 0)
	{
		...
		if (renderParams.BlendMode == AlphaTest)
		{
			RenderAlphaTestPrepassView(RenderEye_Left, layer, frame, depthFrame, numIndices, renderParams);
		}
	...
	if (renderParams.RightFrameIndex >= 0)
	{
		...
		if (renderParams.BlendMode == AlphaTest)
		{
			RenderAlphaTestPrepassView(RenderEye_Right, layer, frame, depthFrame, numIndices, renderParams);
		}
...
void PassthroughRendererDX11::RenderSetupView(const ERenderEye eye, const XrCompositionLayerProjection* layer, std::shared_ptr<CameraGPUFrame> frame, std::shared_ptr<DepthFrame> depthFrame, FrameRenderParameters& renderParams)
{
	...
	if (renderParams.ProjectionMode != Projection_StereoReconstruction && renderParams.bMirrorRender)
	{
		m_renderContext->RSSetState(m_rasterizerStateMirrored.Get());
	}
	else
	{
	...
	vsViewBuffer.worldToHMDProjection = (eye == RenderEye_Left) ? 
		renderParams.HMDEyeLeft.WorldToProjection : 
		renderParams.HMDEyeRight.WorldToProjection;

	XrMatrix4x4f_Invert(&vsViewBuffer.HMDProjectionToWorld, &vsViewBuffer.worldToHMDProjection);

	vsViewBuffer.prevHMDFrame_WorldToHMDProjection = (eye == RenderEye_Left) ? 
		renderParams.PrevHMDFrame_HMDEyeLeft.WorldToProjection : 
		renderParams.PrevHMDFrame_HMDEyeRight.WorldToProjection;

	vsViewBuffer.prevCameraFrame_WorldToHMDProjection = (eye == RenderEye_Left) ? 
		renderParams.ColorHistory_HMDEyeLeft.WorldToProjection :
		renderParams.ColorHistory_HMDEyeRight.WorldToProjection;

	XrMatrix4x4f& hmdViewToWorld = (eye == RenderEye_Left) ? renderParams.HMDEyeLeft.ViewToWorld : renderParams.HMDEyeRight.ViewToWorld;
	XrVector3f inPos{ 0,0,0 };
	XrMatrix4x4f_TransformVector3f(&vsViewBuffer.projectionOriginWorld, &hmdViewToWorld, &inPos);

	vsViewBuffer.disparityUVBounds = GetFrameUVBounds(eye, FrameLayout_StereoHorizontal);
	vsViewBuffer.projectionDistance = renderParams.ProjectionDistance;
	...
	psViewBuffer.bDoCutout = stereoConf.StereoCutoutEnabled && renderParams.ProjectionMode != Projection_StereoReconstruction;
	psViewBuffer.bPremultiplyAlpha = (renderParams.BlendMode == AlphaBlendPremultiplied) && !bCompositeDepth;
	psViewBuffer.bUseFullscreenQuad = renderParams.ProjectionMode == Projection_StereoReconstruction;
...
void PassthroughRendererDX11::RenderDepthPrepassView(const ERenderEye eye, const XrCompositionLayerProjection* layer, std::shared_ptr<CameraGPUFrame> frame, std::shared_ptr<DepthFrame> depthFrame, UINT numIndices, FrameRenderParameters& renderParams)
{
	int swapchainIndex = (eye == RenderEye_Left) ? renderParams.LeftFrameIndex : renderParams.RightFrameIndex;
	int depthSwapchainIndex = (eye == RenderEye_Left) ? renderParams.LeftDepthIndex : renderParams.RightDepthIndex;
	...
	m_renderContext->ClearDepthStencilView(viewData.passthroughDepthStencil[0].DSV.Get(), D3D11_CLEAR_DEPTH, renderParams.bHasReversedDepth ? 0.0f : 1.0f, 0);
	...
	m_renderContext->OMSetDepthStencilState(renderParams.bHasReversedDepth ? m_depthStencilStateGreaterWrite.Get() : m_depthStencilStateLessWrite.Get(), 1);
	...
	if (stereoConf.StereoCutoutEnabled)
	{
		...
		m_renderContext->ClearDepthStencilView(viewData.passthroughDepthStencil[1].DSV.Get(), D3D11_CLEAR_DEPTH, renderParams.bHasReversedDepth ? 0.0f : 1.0f, 0);
...
void PassthroughRendererDX11::RenderMaskedPrepassView(const ERenderEye eye, const XrCompositionLayerProjection* layer, std::shared_ptr<CameraGPUFrame> frame, std::shared_ptr<DepthFrame> depthFrame, UINT numIndices, FrameRenderParameters& renderParams)
{
	int swapchainIndex = (eye == RenderEye_Left) ? renderParams.LeftFrameIndex : renderParams.RightFrameIndex;
	int depthSwapchainIndex = (eye == RenderEye_Left) ? renderParams.LeftDepthIndex : renderParams.RightDepthIndex;
	...
	psViewBuffer.rtArrayIndex = layer->views[viewIndex].subImage.imageArrayIndex;
	psViewBuffer.cameraViewIndex = viewIndex;
	psViewBuffer.bDoCutout = renderParams.ProjectionMode == Projection_StereoReconstruction && stereoConf.StereoCutoutEnabled;
	psViewBuffer.bPremultiplyAlpha = false;
	psViewBuffer.bUseFullscreenQuad = renderParams.ProjectionMode == Projection_StereoReconstruction;
	...
	m_renderContext->OMSetDepthStencilState(GET_DEPTH_STENCIL_STATE(bCompositeDepth, m_configManager->GetConfig_Core().CoreForceMaskedUseCameraImage == renderParams.bHasReversedDepth, bWriteDepth), 1);
	...
	else if (renderParams.ProjectionMode == Projection_Custom2D)
	{
	...
	m_renderContext->PSSetShader(renderParams.ProjectionMode == Projection_StereoReconstruction ? m_maskedAlphaPrepassFullscreenPS.Get() : m_maskedAlphaPrepassPS.Get(), nullptr, 0);

	// Draw with simple vertex shader if we don't need to sample camera
	if (renderParams.ProjectionMode == Projection_StereoReconstruction || (!bCompositeDepth && !m_configManager->GetConfig_Core().CoreForceMaskedUseCameraImage))
	{
		m_renderContext->RSSetState(m_rasterizerState.Get());
	...
	m_renderContext->OMSetDepthStencilState(GET_DEPTH_STENCIL_STATE(false, renderParams.bHasReversedDepth, false), 1);
...
void PassthroughRendererDX11::RenderAlphaPrepassView(const ERenderEye eye, const XrCompositionLayerProjection* layer, std::shared_ptr<CameraGPUFrame> frame, std::shared_ptr<DepthFrame> depthFrame, UINT numIndices, FrameRenderParameters& renderParams)
{
	int swapchainIndex = (eye == RenderEye_Left) ? renderParams.LeftFrameIndex : renderParams.RightFrameIndex;
	int depthSwapchainIndex = (eye == RenderEye_Left) ? renderParams.LeftDepthIndex : renderParams.RightDepthIndex;
	...
	if (bCompositeDepth && renderParams.BlendMode != Additive && renderParams.ProjectionMode != Projection_StereoReconstruction)
	{
	...
	else if (renderParams.BlendMode == AlphaBlendPremultiplied || 
		(renderParams.BlendMode == AlphaBlendUnpremultiplied) || 
		(renderParams.BlendMode == Masked && renderParams.bInvertLayerAlpha))
	{
	...
	else if (renderParams.bInvertLayerAlpha && 
		(renderParams.BlendMode == Additive || renderParams.BlendMode == Opaque) && 
		(renderParams.ProjectionMode != Projection_StereoReconstruction))
	{
	...
	bool bPrepassWriteDepth = renderParams.ProjectionMode != Projection_StereoReconstruction && bWriteDepth;

	m_renderContext->OMSetDepthStencilState(GET_DEPTH_STENCIL_STATE(bCompositeDepth, renderParams.bHasReversedDepth, bPrepassWriteDepth), 1);


	if (renderParams.ProjectionMode == Projection_StereoReconstruction || (renderParams.bInvertLayerAlpha && !bCompositeDepth))
	{
		m_renderContext->IASetPrimitiveTopology(D3D11_PRIMITIVE_TOPOLOGY_TRIANGLESTRIP);
		m_renderContext->VSSetShader(m_fullscreenQuadVS.Get(), nullptr, 0);
...
void PassthroughRendererDX11::RenderAlphaTestPrepassView(const ERenderEye eye, const XrCompositionLayerProjection* layer, std::shared_ptr<CameraGPUFrame> frame, std::shared_ptr<DepthFrame> depthFrame, UINT numIndices, FrameRenderParameters& renderParams)
{
	int swapchainIndex = (eye == RenderEye_Left) ? renderParams.LeftFrameIndex : renderParams.RightFrameIndex;
	...
	DX11TemporaryRenderTarget& tempTarget = GetTemporaryRenderTarget(m_frameIndex, viewIndex);
	...
	ID3D11ShaderResourceView* originalSRVs[5];
	m_renderContext->PSGetShaderResources(0, 5, originalSRVs);
	...
	ID3D11RenderTargetView* nullRTV = nullptr;
	m_renderContext->OMSetRenderTargets(1, &nullRTV, nullptr);
	m_renderContext->PSSetShaderResources(2, 1, tempTarget.SRV.GetAddressOf());
	...
	m_renderContext->PSSetShaderResources(0, 5, originalSRVs);
}
...
void PassthroughRendererDX11::RenderViewModelsForView(const ERenderEye eye, const XrCompositionLayerProjection* layer, std::shared_ptr<CameraGPUFrame> frame, std::shared_ptr<DepthFrame> depthFrame, UINT numIndices, FrameRenderParameters& renderParams)
{
	int swapchainIndex = (eye == RenderEye_Left) ? renderParams.LeftFrameIndex : renderParams.RightFrameIndex;
	int depthSwapchainIndex = (eye == RenderEye_Left) ? renderParams.LeftDepthIndex : renderParams.RightDepthIndex;
	...
	m_renderContext->OMSetDepthStencilState(GET_DEPTH_STENCIL_STATE(bCompositeDepth, renderParams.bHasReversedDepth, bWriteDepth), 1);

	bool bInvertAlpha = renderParams.bInvertLayerAlpha &&
		renderParams.ProjectionMode == Projection_StereoReconstruction &&
		renderParams.BlendMode != Masked;
...
void PassthroughRendererDX11::RenderPassthroughView(const ERenderEye eye, const XrCompositionLayerProjection* layer, std::shared_ptr<CameraGPUFrame> frame, std::shared_ptr<DepthFrame> depthFrame, UINT numIndices, FrameRenderParameters& renderParams)
{
	int swapchainIndex = (eye == RenderEye_Left) ? renderParams.LeftFrameIndex : renderParams.RightFrameIndex;
	int depthSwapchainIndex = (eye == RenderEye_Left) ? renderParams.LeftDepthIndex : renderParams.RightDepthIndex;
	...
	bool bDepthWrittenInPrepass = renderParams.ProjectionMode != Projection_StereoReconstruction &&
		(renderParams.BlendMode == Opaque && !renderParams.bEnableDepthBlending) ||
		(renderParams.BlendMode == Additive) ||
		(renderParams.ProjectionMode != Projection_StereoReconstruction &&
			renderParams.BlendMode != Masked && renderParams.bEnableDepthBlending);
	...
	if (bCompositeDepth && renderParams.ProjectionMode == Projection_StereoReconstruction && renderParams.BlendMode != Additive)
	{
		m_renderContext->OMSetBlendState(m_blendStateSrcAlpha.Get(), nullptr, UINT_MAX);
	}
	else if ((renderParams.BlendMode == AlphaBlendPremultiplied && 
		(!bCompositeDepth || renderParams.ProjectionMode == Projection_StereoReconstruction)) ||
		renderParams.BlendMode == Additive)
	{
		m_renderContext->OMSetBlendState(bInvertAlpha ? m_blendStateInvDestAlphaPremultiplied.Get() : m_blendStateDestAlphaPremultiplied.Get(), nullptr, UINT_MAX);
	}
...
void PassthroughRendererDX11::RenderBackgroundForView(const ERenderEye eye,  const XrCompositionLayerProjection* layer, std::shared_ptr<CameraGPUFrame> frame, std::shared_ptr<DepthFrame> depthFrame,  UINT numIndices, FrameRenderParameters& renderParams)
{
	int swapchainIndex = (eye == RenderEye_Left) ? renderParams.LeftFrameIndex : renderParams.RightFrameIndex;
	int depthSwapchainIndex = (eye == RenderEye_Left) ? renderParams.LeftDepthIndex : renderParams.RightDepthIndex;
	...
	m_renderContext->IASetIndexBuffer(m_cylinderMeshIndexBuffer.Get(), DXGI_FORMAT_R32_UINT, 0);
	m_renderContext->VSSetShader(m_passthroughVS.Get(), nullptr, 0);
	m_renderContext->RSSetState(renderParams.bMirrorRender ? m_rasterizerStateDepthBiasMirrored.Get() : m_rasterizerStateDepthBias.Get());
	...
	m_renderContext->OMSetDepthStencilState(GET_DEPTH_STENCIL_STATE(bCompositeDepth, renderParams.bHasReversedDepth, depthConfig.DepthWriteOutput && depthConfig.DepthReadFromApplication), 1);
