...
float sobel_discontinuity_adjust(in Texture2D<float> tex, in float depth, in float2 uvs, out bool bWasFiltered)
{
    ...
    if(magnitude > 0.15)
    {
        ...
        //float nearest = tex.Load(int3(pixelPos, 0));
        
        //float pixelEdgeFactor = clamp((abs(texturePos.x - pixelPos.x - 0.5) * 2 + abs(texturePos.y - pixelPos.y - 0.5) * 2), 0.0, 1.0);
        //float2 pixelOffset = clamp(float2((texturePos.x - pixelPos.x - 0.5) * 2, (texturePos.y - pixelPos.y - 0.5)) * 2, -1.0, 1.0);

        //float subpixelFactor = saturate(dot(normalize(float2(-filterX, -filterY)), pixelOffset));
        //return subpixelFactor;
        //return tex.Load(int3(round(texturePos + float2(dispL - dispR, dispU - dispD) * 10.0), 0));
        
        ////return lerp(nearest, maxDepth, saturate(dot(float2(filterX, filterY), -pixelOffset)));
        //if(dot(float2(filterX, filterY), -pixelOffset) > 0)
        //{
        //    return maxDepth;
        //}
        //else
        ...
        //float adjustedDepth = g_depthMap.Sample(g_samplerState, uvs + pixelOffset * invTexSize);
        ...
        for (int x = -GAUSSIAN_FILTER_WIDTH; x <= GAUSSIAN_FILTER_WIDTH; x++)
        {
            ...
            for (int y = -GAUSSIAN_FILTER_WIDTH; y <= GAUSSIAN_FILTER_WIDTH; y++)
            {
                ...
                totalWeight += weight;
                outDepth += g_depthMap.SampleLevel(g_samplerState, uvs + float2(x, y) * invTexSize, 0) * weight;
            }
        }

        float adjustedDepth = outDepth / totalWeight;
        
        bool inForeground = ((maxDepth - adjustedDepth) > (adjustedDepth - minDepth));
        
        //float offsetFactor = saturate((1 - pixelEdgeFactor) * 10.0 * length(float2(filterX, filterY)));
        float offsetFactor = saturate(10.0 * magnitude);
        
        return lerp(depth, inForeground ? minDepth : maxDepth, offsetFactor);
        
        
        //return inForeground ? 0.5 - 0.5 * offsetFactor : 0.5 + 0.5 * offsetFactor;
        
        //float4 samples = tex.Gather(g_samplerState, uvs);
        //
        
        //float minSample = min(samples.x, min(samples.y, min(samples.z, samples.w)));
        //float maxSample = max(samples.x, max(samples.y, max(samples.z, samples.w)));
        //float halfDepth = lerp(minSample, maxSample, 0.5);
        

        //int numBackground = (samples.x > halfDepth) ? 1 : 0;
        //numBackground += (samples.y > halfDepth) ? 1 : 0;
        //numBackground += (samples.z > halfDepth) ? 1 : 0;
        //numBackground += (samples.w > halfDepth) ? 1 : 0;
        
        //float halfDepth = lerp(minDepth, maxDepth, 0.5);
        
        //int numBackground = (dispU > halfDepth) ? 1 : 0;
        //numBackground += (dispD > halfDepth) ? 1 : 0;
        //numBackground += (dispL > halfDepth) ? 1 : 0;
        //numBackground += (dispR > halfDepth) ? 1 : 0;
        
        //numBackground += (dispUL > halfDepth) ? 1 : 0;
        //numBackground += (dispDL > halfDepth) ? 1 : 0;
        //numBackground += (dispUR > halfDepth) ? 1 : 0;
        //numBackground += (dispDR > halfDepth) ? 1 : 0;
        
        //numBackground += (nearest > halfDepth) ? 1 : 0;
            
        //if (numBackground > 5)
        //{
        //    return maxDepth;
        //}
        //else if (numBackground < 5)
        //{
        //    return minDepth;
        //}
        //else
...
PS_Output main(VS_OUTPUT input)
{
    ...
    //float depth = LoadTextureNearestClamped(g_depthMap, screenUvs);
    //float4 cameraValidation = LoadTextureNearestClamped(g_cameraValidation, screenUvs);
    ...
    if (true)//projectionConfidence < 0.5)
    {
        depth = sobel_discontinuity_adjust(g_depthMap, depth, screenUvs, bIsDiscontinuityFiltered);
        //float4 samples = g_depthMap.Gather(g_samplerState, screenUvs);
        //float nearest = LoadTextureNearestClamped(g_depthMap, screenUvs);
        
        //float minDepth = min(samples.x, min(samples.y, min(samples.z, samples.w)));
        //float maxDepth = max(samples.x, max(samples.y, max(samples.z, samples.w)));
        //float halfDepth = lerp(minDepth, maxDepth, 0.5);
        
        //if ((maxDepth - minDepth) > 0.01)
        //{
        //    int numBackground = (samples.x > depth) ? 1 : 0;
        //    numBackground += (samples.y > depth) ? 1 : 0;
        //    numBackground += (samples.z > depth) ? 1 : 0;
        //    numBackground += (samples.w > depth) ? 1 : 0;
